...
function RifleManModel({ scene }) {
  ...
  useEffect(() => {
    ...
    loader.load(
      'glTF-models/dwarfrifleman_m1/dwarfrifleman_m_03105,209,301,402,501.gltf',
      (gltf) => {
        const animations = gltf.animations;
        const rifleMan = gltf.scene;
        modelRef.current = rifleMan;
        
        rifleMan.traverse((child) => {
          if (child.isMesh) {
            child.name = child.name || 'BronzebeardMesh';
          }
        });
        
        const mixer = new THREE.AnimationMixer(rifleMan);
        mixerRef.current = mixer;
        rifleMan.position.set(-531, 9.8, 364); 
        rifleMan.rotation.set(0, Math.PI / 1, 0);
        rifleMan.scale.set(2, 2, 2);
        scene.add(rifleMan);
      ...
      undefined,
      (error) => {
        console.error('An error happened while loading rifleMan:', error);
      }
    );
